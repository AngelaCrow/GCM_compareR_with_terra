...
presente <- terra::rast("WC/wc2.1_30s_bio_Mexico.tif")
names(presente)
...
dir.create(paste0("/Volumes/TOSHIBA EXT/COBERTURAS/GIZ_GCM_compareR/WC/",ruta_futuro,"/deltas"), showWarnings = FALSE)
...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/GIZ_GCM_compareR/WC/ssp126_2021-2040/deltas/d.", basename(futuro_path))
...
futuro_files <- list.files("WC/ssp126_2021-2040/deltas/", full.names = TRUE, pattern = ".tif$")
...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  # Guardar el raster de diferencia
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/GIZ_GCM_compareR/WC/ssp370_2021-2040/deltas/d.", basename(futuro_path))
  terra::writeRaster(diferencia, nombre_salida, overwrite = TRUE)
}
...
#Tabla de las media globales de cada delta a futuro
futuro_files <- list.files("WC/ssp370_2021-2040/deltas/", full.names = TRUE, pattern = ".tif$")
...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  # Guardar el raster de diferencia
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/GIZ_GCM_compareR/WC/ssp585_2021-2040/deltas/d.", basename(futuro_path))
  terra::writeRaster(diferencia, nombre_salida, overwrite = TRUE)
}
...
#Tabla de las media globales de cada delta a futuro
futuro_files <- list.files("WC/ssp585_2021-2040/deltas/", full.names = TRUE, pattern = ".tif$")
...
write.csv(all_data, "WC/Mean_deltasBio_2021-2040.csv", row.names = F)