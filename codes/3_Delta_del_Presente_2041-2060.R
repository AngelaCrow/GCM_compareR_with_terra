...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/WC/ssp126_2041-2060/deltas/d.", basename(futuro_path))
...
futuro_files <- list.files(paste0(ruta,"/ssp126_2041-2060/deltas/"), full.names = TRUE, pattern = ".tif$")

results1 <- map_dfr(futuro_files, ~ {
  modelo <- str_extract(.x, "(?<=wc2\\.1_30s_bioc_)[^_]+(?=_ssp126_2041-2060)")
  procesar_raster(.x, maskara, modelo, "meanFuture")
}) %>%
  dplyr::mutate(Year = "2041-2060", 
                SSP = "SSP1 2.6",
                Tipo = "deltas_ssp126_2041-2060")
...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  # Guardar el raster de diferencia
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/WC/ssp370_2041-2060/deltas/d.", basename(futuro_path))
  terra::writeRaster(diferencia, nombre_salida, overwrite = TRUE)
}
...
#Tabla de las media globales de cada delta a futuro
futuro_files <- list.files(paste0(ruta,"/ssp370_2041-2060/deltas/"), full.names = TRUE, pattern = ".tif$")

results2 <- map_dfr(futuro_files, ~ {
  modelo <- str_extract(.x, "(?<=wc2\\.1_30s_bioc_)[^_]+(?=_ssp370_2041-2060)")
  procesar_raster(.x, maskara, modelo, "meanFuture")
}) %>%
  dplyr::mutate(Year = "2041-2060", 
                SSP = "SSP3 7.0",
                Tipo = "deltas_ssp370_2041-2060")
...
calcular_diferencias <- function(presente, futuro_path, maskara) {
  ...
  # Guardar el raster de diferencia
  nombre_salida <- paste0("/Volumes/TOSHIBA EXT/COBERTURAS/WC/ssp585_2041-2060/deltas/d.", basename(futuro_path))
  terra::writeRaster(diferencia, nombre_salida, overwrite = TRUE)
}
...
#Tabla de las media globales de cada delta a futuro
futuro_files <- list.files(paste0(ruta,"/ssp585_2041-2060/deltas/"), full.names = TRUE, pattern = ".tif$")

results3 <- map_dfr(futuro_files, ~ {
  modelo <- str_extract(.x, "(?<=wc2\\.1_30s_bioc_)[^_]+(?=_ssp585_2041-2060)")
  procesar_raster(.x, maskara, modelo, "meanFuture")
}) %>%
  dplyr::mutate(Year = "2041-2060", 
                SSP = "SSP5 8.5",
                Tipo = "deltas_ssp585_2041-2060")

# Unir los resultados de cada ssp
all_data<-rbind(results1, results2, results3)
# Guardar los resultados
write.csv(all_data, paste0(ruta,"/Mean_deltasBio_2041-2060.csv"), row.names = F)
